...
int builtin_cmd(char **argv) 
{
    if (argv != NULL && *argv != 0)
    {
        if (!strcmp(*argv, BUILTIN_CMD_JOBS))
        {
            listjobs(jobs);
            fflush(stdout);
            return 1;          
        ...
        if (!strcmp(*argv, BUILTIN_CMD_QUIT))
        {
            exit(0);
        }
        else 
        if (!strcmp(*argv, BUILTIN_CMD_BG))
        {
            do_bgfg(argv);
            return 1; 
        }
        else 
        if (!strcmp(*argv, BUILTIN_CMD_FG))
        {
            do_bgfg(argv);
            return 1; 
        } 
...
void do_bgfg(char **argv) 
{
    ...
    else
    {
       ...
       if (!strcmp(argv[0], BUILTIN_CMD_BG)) {
           printf("[%d] (%d) %s\n", job->jid,job->pid,job->cmdline);
           job->state = BG;
       }
       else if (!strcmp(argv[0], BUILTIN_CMD_FG)) {
           job->state = FG;
           waitfg(job->pid);
       }
    }
